 ...
 * index.html and would otherwise get the same generic wallet preview. Someone
 * sharing their card would appear in the chat as "BuhoGO - NWC, LNBits and
 * Spark ready wallet app", which tells the recipient nothing about who sent it.
 *
...
const OG_IMAGE = '/og/profile-card.png';
...
export default async function profilePreview(request, context) {
  ...
  try {
    ...
    const patched = html
      ...
      .replace(/<meta name="title"[^>]*>/i, `<meta name="title" content="${escapeAttr(title)}"/>`)
      .replace(/<meta name="description"[^>]*>/i, `<meta name="description" content="${escapeAttr(description)}"/>`)
      .replace(/<meta property="og:url"[^>]*>/i, `<meta property="og:url" content="${escapeAttr(canonical)}"/>`)
      .replace(/<meta property="og:title"[^>]*>/i, `<meta property="og:title" content="${escapeAttr(title)}"/>`)
      .replace(/<meta property="og:description"[^>]*>/i, `<meta property="og:description" content="${escapeAttr(description)}"/>`)
      .replace(/<meta property="og:image"[^>]*>/i, `<meta property="og:image" content="${escapeAttr(image)}"/>`)
      .replace(/<meta property="twitter:url"[^>]*>/i, `<meta property="twitter:url" content="${escapeAttr(canonical)}"/>`)
      .replace(/<meta property="twitter:title"[^>]*>/i, `<meta property="twitter:title" content="${escapeAttr(title)}"/>`)
      .replace(/<meta property="twitter:description"[^>]*>/i, `<meta property="twitter:description" content="${escapeAttr(description)}"/>`)
      .replace(/<meta property="twitter:image"[^>]*>/i, `<meta property="twitter:image" content="${escapeAttr(image)}"/>`);
